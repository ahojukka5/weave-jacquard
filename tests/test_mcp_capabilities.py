...
def test_public_capabilities_have_unique_dependency_order() -> None:
    ...
    assert names.index("merge_test_impact") < names.index("policy")
...
def test_public_install_loads_modules_in_order_and_replaces_help_once() -> None:
    ...
    def loader(name: str) -> ModuleType:
        loaded.append(name)
        if name == "weave_frontend.mcp_merge_test_impact_guidance":
    ...
    assert loaded == [
        "example.base",
        "example.feature",
        "weave_frontend.mcp_merge_test_impact_guidance",
    ]
    ...
    assert "merge candidate test impact plans" in server.descriptions["weave_help"]
