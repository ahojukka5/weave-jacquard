...
def test_raw_snapshot_accepts_exact_limit_and_rejects_limit_plus_one(
    ...
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(b"WJR1" + b"x" * 17))
...
def test_zlib_output_limit_rejects_small_decompression_bomb(
    ...
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(blob))
...
def test_zlib_stream_rejects_truncation_and_trailing_bytes() -> None:
    ...
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(b"WJZ1" + payload[:-1]))
        == "SNAPSHOT_COMPRESSION_TRUNCATED"
    )
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(b"WJZ1" + payload + b"junk"))
...
def test_snapshot_rejects_prefix_utf8_json_tree_and_hash_corruption() -> None:
    ...
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(b"BAD!payload"))
        == "SNAPSHOT_ENCODING_INVALID"
    )
    assert (
        _error_code(lambda: codec.decompress_snapshot_json(b"WJR1\xff")) == "SNAPSHOT_UTF8_INVALID"
    )
    assert (
        _error_code(lambda: codec.decode_snapshot_text("{", expected_hash=expected_hash))
    ...
    assert (
        _error_code(lambda: codec.decode_snapshot_text(canonical, expected_hash="0" * 64))
    ...
    assert (
        _error_code(
            lambda: codec.load_revision_state(
                connection,
                "revision",
                expected_root_hash=codec.hash_value({"a.weave": first, "b.weave": second}),
            )
        )
